...
public OnFilterScriptInit(){
    print("\n--------------------------------------");
    print(" CamEditor by Drebin (Modified)");
    print("--------------------------------------\n");
    return 1;
}
...
public OnPlayerConnect(playerid){
    ...
    EditorMode[playerid]             = MODE_INTERPOLATE;
    return 1;
}

public OnPlayerSpawn(playerid){
    if(!IsCreating[playerid]) SendClientMessage(playerid, -1, "Type /cameditor to open the camera movement editor");
    return 1;
}
    ...
    if(!strcmp(cmdtext, "/cameditor", true)){
        if(!IsCamMoving[playerid]){
            if(GetPVarType(playerid, "FlyMode")){
                ...
                ShowPlayerDialog(playerid, DIALOG_CAM_TYPE, DIALOG_STYLE_LIST, "Choose Camera Type", "Interpolate Camera (Move)\nStatic Camera (Static)", "Choose", "Cancel");
            }
        }
        return 1;
    }
...
public OnPlayerKeyStateChange(playerid, KEY:newkeys, KEY:oldkeys){
    if((newkeys & KEY_FIRE) && !(oldkeys & KEY_FIRE)){
        if(IsCreating[playerid]){
            if(EditorMode[playerid] == MODE_STATIC){
                 ...
                 SendClientMessage(playerid, -1, "{8EFF8E}>{FFFFFF} Static position {8EFF8E}set.");
                 ShowPlayerDialog(playerid, DIALOG_EXPORTNAME, DIALOG_STYLE_INPUT, "Save Static Camera", "Enter a name for the static camera view", "Ok", "Cancel");
                 return 1;
            }
            ...
            if(SettingFirstLoc[playerid]){
                ...
                if(IsReSettingStart[playerid]){
                    ...
                    SendClientMessage(playerid, -1, "Now use {F58282}LMB {FFFFFF}to save the camera's position as the {F58282}end {FFFFFF}position.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]){
    switch(dialogid)
    {
        case DIALOG_CAM_TYPE:{
            if(response){
                if(listitem == 0){ // Interpole
                    EditorMode[playerid] = MODE_INTERPOLATE;
                    FlyMode(playerid);
                }
                else if(listitem == 1){ // Static
                    EditorMode[playerid] = MODE_STATIC;
                    FlyMode(playerid);
                }
            }
        }
            ...
            if(response){
                switch(listitem){
                    ...
                    case 1: { // Change Start
                        ...
                        SendClientMessage(playerid, -1, "Use {F58282}LMB {FFFFFF}to set a new {F58282}start {FFFFFF}position.");
                    }
                    case 2: { // Change End
                        DestroyPlayerObject(playerid, noclipdata[playerid][flyobject]);
                        IsReSettingStart[playerid] = false;
                        ...
                        SetPVarInt(playerid, "FlyMode", 1);
                        noclipdata[playerid][cameramode] = CAMERA_MODE_FLY;
                        SendClientMessage(playerid, -1, "Use {F58282}LMB {FFFFFF}to set a new {F58282}end {FFFFFF}position.");
                    }
                    case 3: { // Change Speed
                        new string[512];
                        format(string, sizeof(string), "Please enter the desired {F58282}movement{a9c4e4} time in milliseconds\n\nCurrent movement speed: \t{F58282}%i milliseconds\n{a9c4e4}Current rotation speed: \t{F58282}%i milliseconds\n\n\nNote: {a9c4e4}1 second = 1000 milliseconds", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
        ...
        case DIALOG_CLOSE_NEW:
        {
            if(response) {
                // Reset
                IsCreating[playerid]      = true;
                if(EditorMode[playerid] == MODE_INTERPOLATE) SettingFirstLoc[playerid] = true;
                FlyMode(playerid); 
            }
            else{
                SendClientMessage(playerid, -1, "You exited the camera movement editor.");
...
forward ExportMovement(playerid, const inputtext[]);
public ExportMovement(playerid, const inputtext[]){
    new tagstring[64];
    new movestring[512];
    new rotstring[512];
    new filename[50];
    ...
    if(EditorMode[playerid] == MODE_INTERPOLATE){
        ...
        format(rotstring, sizeof(rotstring), "SetPlayerCameraLookAt(playerid, %f, %f, %f);", coordInfo[playerid][StartLookX], coordInfo[playerid][StartLookY], coordInfo[playerid][StartLookZ]);
    }
...
stock GetNextCameraPosition(move_mode, const Float:CP[], const Float:FV[], &Float:X, &Float:Y, &Float:Z){
...
stock FlyMode(playerid){
    ...
    if(EditorMode[playerid] == MODE_INTERPOLATE){
        SettingFirstLoc[playerid] = true;
        SendClientMessage(playerid, -1, "You entered the {FFFF00}Interpolate Camera {FFFFFF}editor.");
        SendClientMessage(playerid, -1, "Use {F58282}LMB {FFFFFF}to save the {F58282}start {FFFFFF}position.");
    }
    else{ // Mode Static 
        SendClientMessage(playerid, -1, "You entered the {FFFF00}Static Camera {FFFFFF}editor.");
        SendClientMessage(playerid, -1, "Use {F58282}LMB {FFFFFF}once to save the static position.");
    }
    ...
    SendClientMessage(playerid, -1, "With {F58282}W, S, A, D{FFFFFF} you can move the camera.");
    
    return 1;
}

stock IsNumeric(const szInput[]){
    new iChar, i = 0;
